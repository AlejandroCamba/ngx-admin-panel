...
export class AuthInterceptor implements HttpInterceptor {
    ...
    intercept(req: HttpRequest<any>, next: HttpHandler): Observable<HttpEvent<any>> {
        ...
        if (req.url.indexOf('{{base}}') !== -1 && req.url.indexOf('{{loginEndpoint}}') !== -1) {
            ...
            url = req.clone({
                url: req.url
                    .replace('{{base}}', this.environemt.apiUrl)
                    .replace('{{loginEndpoint}}', this.environemt.endpoints.login.path),
        ...
        if (req.url.indexOf('{{base}}') !== -1 && req.url.indexOf('{{logoutEndpoint}}') !== -1) {
            url = req.clone({
                url: req.url
                    .replace('{{base}}', this.environemt.apiUrl)
                    .replace('{{logoutEndpoint}}', this.environemt.endpoints.logout.path),
            });
            console.log(url);
        }
