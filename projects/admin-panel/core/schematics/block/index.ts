...
export function blockComponent(options: MyBlockSchema): Rule {
  return (tree: Tree) => {
    ...
    if (options.adminView) {
      ...
      const content2Append = `<admin-${strings.dasherize(options.name)}></admin-${strings.dasherize(options.name)}>`;
      ...
      tree.overwrite(
        `src/app/${strings.dasherize(options.adminView)}-view/${strings.dasherize(options.adminView)}-view.component.html`,
        updatedContent
      );
    ...
    const templateSource = apply(url('./files'), [
      ...
      move(normalize((options.adminView ? `src/app/${strings.dasherize(options.adminView)}-view/tabs` : options.path) as string))
    ]);
