...
@Component({
    ...
    template: `
        ...
        <ng-template #nextpage></ng-template></div>
...
export class TableComponent extends BlockComponent implements OnInit {
    ...
    */
    @ViewChildren('tablecontent', { read: ViewContainerRef }) tableElementsContainer: QueryList<
        ViewContainerRef
    >;
    ...
    */
    @ViewChild('nextpage', { read: ViewContainerRef }) nextPageContainer: ViewContainerRef;
    ...
    updateValue(value, cell, rowIndex) {
        ...
        this.tableData[rowIndex][cell] = value ? value : this.tableData[rowIndex][cell];
        this.rows = [...this.rows];
    }
    ...
    comopnentFactoryStorage(
        ...
        const factories: Array<ComponentFactory<unknown> | ComponentFactory<unknown>[]> = [];
        for (let i = 0; i < args.length; i++) {
            if (args[i] instanceof Array) {
                const subComponentArray = [];
                for (let j = 0; j < args[i].length; j++) {
                    subComponentArray.push(this.resolver.resolveComponentFactory(args[i][j]));
                }
            ...
            .subscribe((labels: []) => {
                ...
                const creacteComponentWithFactory = (
                    ...
                    if (elementIndex !== undefined) {
                        adapterRef = container.createComponent(adapterFactory, elementIndex);
                    } else {
                ...
                for (let i = 0; i < labels.length; i++) {
                    ...
                    for (let j = 0; j < this.tableData.length; j++) {
                        ...
                        if (factories[i] instanceof Array) {
                            ...
                            const mainAdapterDOMLocation = mainAdapterRef.location.nativeElement;
                            ...
                            this.renderer.listen(
                                ...
                                () => {
                                    if (
                                        ...
                                        this.tableConfig
                                            .workflow.columns[i]['nextPage'].withInputValues.forEach(property => {
                                                console.log(this.tableData[j-pivot][property.name])
                                                if (property.values) {
                                                    component.instance[property.name] = property.values;
                                                    component.instance['selectedValue'] = this.tableData[j-pivot][property.name];
                                                } else {
                                                    component.instance[property.name] = this.tableData[j-pivot][property.name];
                                                }
                                            });



                                        component.instance['id'] = this.tableData[j]['id']; // asignacion de id para el fetch con datos de formulario
                                        component.instance['middlePagerEmitter'].subscribe((value) => {
                                            component.destroy();
                            ...
                            if (this.tableConfig.workflow.columns[i].mutate !== 'none') {
                                this.renderer.listen(
                                    ...
                                    (event) => {
                                        ...
                                        const mutatedComponentInstance = mutatedAdapterInstance.createComponent(
                                            this.resolver.resolveComponentFactory(
                                                this.tableConfig.workflow.columns[i].into
                                            )
                                        ).instance as IndependentComponent;
